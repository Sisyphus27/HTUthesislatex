...
### 9.13 无编号章页眉 mark（`\chapter*` 须手动 `\markboth`）

spec §2.5「奇数页页眉 = 一级标题」依赖 fancyhdr `\rightmark`（cls:358 `\fancyhead[CO]{\rightmark}`）。`\rightmark` 由各章 `\chaptermark`/`\@mkboth` 写入。

- **编号章** `\chapter{}`：ctex `\@chapter` 自动调 `\chaptermark{标题}`（cls:371-377 重定义为 `\markboth`）→ 页眉自动正确。
- **无编号章** `\chapter*{}`：ctex `\@schapter` **不**调 `\chaptermark`/`\@mkboth` → `\rightmark` 不更新 → 奇数页页眉**残留前一章名**（如绪论残留前置"主要符号对照表"）。

**正确写法**（`\chapter*` 后补 `\markboth{标题}{标题}`）：

```latex
\chapter*{绪论}
\markboth{绪论}{绪论}        % 设 running mark
\addcontentsline{toc}{chapter}{绪论}
```

`\markboth{绪论}{绪论}` 复刻 `\chaptermark` 对无编号章的行为（`\CTEXifname` 假 → 裸标题，不前缀章号）。

**已知陷阱**：勿在 `\mainmatter` 用 `\markboth{}{}` 清洗前置残留 mark——`\clearpage` 不运走 mark-only 页，空 mark 会浮到正文首章首页成为该页 `\firstmark`，致首页页眉空（与首章自身 `\markboth` 冲突）。前置残留改由各章自身 `\markboth` 覆盖即可（cls `\mainmatter` 处 `[基础]` 注释钉此 gotcha）。模板自研 `\htu@chapter*`（cls:586）同样把 `\@mkboth` 放章后，故也有此首页时序缺陷——无编号章统一用上面的"`\chapter*` + `\markboth`"两行写法。
