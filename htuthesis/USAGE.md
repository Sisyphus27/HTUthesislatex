...
### 9.13 无编号章页眉 mark（M6 自动兜底）

spec §2.5「奇数页页眉 = 一级标题」依赖 fancyhdr `\rightmark`（cls `\fancyhead[CO]{\rightmark}`）。`\rightmark` 由各章的 `\chaptermark`/`\@mkboth` 写入。模板**三类章命令现在都自动设 mark**（Story 6.1 M6，2026-06-24）：

- **编号章** `\chapter{标题}`：ctex `\@chapter` 自动调 `\chaptermark` → 页眉自动正确。
- **无编号章** `\chapter*{标题}`：M6 包装 `\chapter`，星号形追加 `\@mkboth{标题}{标题}`（cls `\NewCommandCopy{\htu@orig@chapter}{\chapter}` + `\RenewDocumentCommand{\chapter}{s o m}`）→ 页眉自动正确。**无需手写 `\markboth`。**
- **模板原语** `\htu@chapter*`（前置摘要/目录/符号表 + 后置致谢/参考文献/声明）：cls 自身 `\@mkboth` → 页眉自动正确。

**绪论写法**（直接用原生 `\chapter*`，M6 自动兜底页眉）：

```latex
\chapter*{绪论}
\addcontentsline{toc}{chapter}{绪论}   % 若需入目录
```

**页眉规则**：奇数页（CO）= 当前一级标题（`\rightmark`）；偶数页（CE）= 论文题目（`\ctitle`）。页眉只在 `\mainmatter`（正文）显示——前置/后置按 spec §2.5「从主体部分开始」无页眉（§9.12）。页眉只反映**章**级，不反映节（`\sectionmark` 被抑制）。

**M6 ≠ S2 清洗**：M6 加的是**实 mark**（`\@mkboth{绪论}{绪论}`），非空 mark 清洗。勿在 `\mainmatter` 用 `\markboth{}{}` 清洗前置残留——`\clearpage` 不运走 mark-only 页，空 mark 会浮到正文首章首页致页眉空（记忆 `mainmatter-mark-clear-breaks-chapter-firstpage`；cls `\mainmatter` 处 `[基础]` 注释钉此 gotcha）。M6 设计正是让章自身设实 mark 覆盖陈旧值，而非清洗。

**历史**：M6 前（v1.0.0/v1.1.1）原生 `\chapter*` 不设 mark，绪论须手写 `\markboth{绪论}{绪论}`。该实例级修复未进模板 → 新实例复发。Story 6.1 M6 把兜底提升至模板级，根因消除（见 `brainstorming-session-2026-06-24-154931.md` + architecture.md §原生 `\chapter*` header-mark 兜底）。

### 9.14 同源脚注写作规范（非模板 bug）

case-2（页下注，每页重编号，§2.14 + §1.2.4）下，同一句话中对同一文献 `\footfullcite{key}` 多次调用会产生**多条完全相同的完整脚注**（如一句三连引 → 页下 [1][2][3] 三条全同）。这是**写作规范问题，非模板缺陷**——模板按设计工作，非 bug。

**(a) 机制：每个 `\footfullcite` 是独立引用事件。** case-2 每页重编号，每次调用独立产一条完整 GB/T 7714 脚注；同源重复调用 = 重复完整条目。参考论文 PDF p16（中国政府网 二十大报告）同样 `[1]`/`[2]` 重复全条、不去重——实态即如此。spec §2.14 case-2 对「同页同源」沉默，不强制去重（§2.14 case-1 line 289「一个文献一个编号」是 case-1 原则，不适用 case-2）。

**(b) 写作规范（作者侧）：**
- 同一文献在相近语境（一句/一段）**引用一次**即可；
- 若需区分同一文献的不同页码/论点，**首引完整** `\footfullcite{key}` + 后续手写「**前注[N]，第M页**」（相邻可用「同上」/ibid）；
- **避免一句之内对同一文献 `\footfullcite` 多次**（v1.1.1 示例 `chap01:127` 对 `YangHua2021` 三连引即此反模式）。

**(c) 模板故意不去重（B-2 方案已拒，6 风险）。** 自动去重（编号复用 `[N]`）经 v1.1.1 实测暴露 6 个不可接受风险：tracker 选项 cls 上下文失效（R1）、文档域 label 碰撞（R2，39 处重复定义）、脚注正文键名泄漏（R3）、去重未触发（R4）、**R5 = biblatex 架构天花板**（无「同键同页任意位置」原语，仅 `\ifciteseen` 文档域 / `\ifciteibid` 相邻 / `\ifsamepage`；非相邻同页 A…A 检测不到）、`\label` 捕获脚注号脆弱（R6）。R5 不可绕过，故模板**故意不去重**——由作者按 (b) 规范避免重复引用。详见 `brainstorming-session-2026-06-24-154931.md` Bug 3 + architecture.md §同源脚注重复裁定。

### 9.15 多重引用（多键 `\footfullcite{a,b}`）（非模板 bug）

case-2（页下注）下，**一次** `\footfullcite{key1,key2,...}` 多键调用只产生**一个**脚注标记 `[N]`，但该脚注正文**内含全部所引文献的完整 GB/T 7714 条目**，各条目以 `\multicitedelim`（逗号）分隔。即 body 仅见单 `[N]`、页下脚注却列出多篇——**非"丢了引用"，是 biblatex 默认合并语义**，非模板 bug。

**(a) 机制：一次多键 = 一条脚注、多篇条目。** biblatex `\footfullcite{a,b}` 把多个键的完整著录合并进同一 `\footnote`，共用一个序号 `[N]`。默认 `sortcitations=true`，脚注内条目按 nyt（名-年-题）重排，**不保留源码键顺序**。实测 v1.2.0 `chap02:14`：`\footfullcite{zhoula2021,xi2025}` → 页下 `[1] 习近平. 习近平经济文选...2025: 297-298, 周黎安. 地区增长联盟...2021, 41(6): 1-40.`（xi2025 被 nyt 排前，两篇俱全）。

**(b) spec 真值源：case-2 对多重引用格式沉默。** spec §1.2.4 line 99「同一处引用多篇文献时，应将各篇文献的序号在方括号中全部列出，各序号间用『，』。如遇连续序号，起讫序号间用短横线连接」是 **case-1 顺序编码制**规则（示例 2 line 97：`斐伟[570, 83]`、`[255-256]`）；line 109「若文中引文文献采用页下注（脚注）的形式，每页重新编号」是 case-2 的全部规定，**对多重引用格式沉默**。故 case-2 把多篇合并进单 `[N]` 脚注既不违反 spec，又与 case-1「一处引用、方括号内列多篇」的哲学一致（GB/T 7714-2015 亦允许多条文献共用一个引用标注）。truth 源层次（CLAUDE.md）：spec 优先，spec 沉默处实现自由。

**(c) 作者选项（按需选一）：**
- **保持合并（默认，多数场景推荐）**：`\footfullcite{a,b}` —— 单标记 `[N]` + 页下多篇完整条目。读者看页下即见全部所引。
- **要分离标记 `[1][2]`（每篇独立脚注）**：拆成相邻两次调用 `\footfullcite{a}\footfullcite{b}`（不同源可拆；**同源**拆分会触发 §9.14 重复全条反模式，勿为）。
- **要保留源码键顺序**（不按 nyt 重排）：加 biblatex 选项 `\ExecuteBibliographyOptions{sortcitations=false}`（标准选项，按 `\footfullcite{}` 内键序排列；改前建议编译验证）。
...
| 页眉每页交错、不一致（偶数页是论文题目、奇数页是章标题） | 正常，非 bug。spec §2.5 奇偶页眉设计：偶数页页眉=论文题目，奇数页页眉=一级标题，均五号宋体（双面打印装订设计） | 无需处理，属正常现象 |
| 无编号章（`\chapter*`，如绪论）奇数页页眉显示**前一章名**（如"主要符号对照表"）而非本章名 | ctex 星号章 `\@schapter` 不调 `\chaptermark`/`\@mkboth` → `\rightmark` 不更新、残留前置 mark | `\chapter*{绪论}` 后补 `\markboth{绪论}{绪论}`（见 §9.13）。编号章 `\chapter{}` 无此患 |
| 用了 `\footcite{key}` 但页下没有完整著录（只有上标数字） | case-2（页下注）须用 `\footfullcite{key}`；`\footcite` 在 numeric 样式只产生 `[N]` 上标编号，无页下完整 GB/T 7714 条目 | 改 `\footcite`→`\footfullcite`，重编译（§9.3） |
| 目录页码滞后正文（如目录标 p49，实际 p59） | 编译遍数不足。目录（.toc）需 xelatex ≥2 遍收敛 | 重跑 `latexmk -xelatex main` 或 `make thesis`（自动多遍）；勿单跑 `xelatex main`（不重跑 biber + 遍数不够） |
...
**Q: 绪论（`\chapter*`）的奇数页页眉为什么显示成"主要符号对照表"之类的**前一章**名？**
A: ctex 星号章 `\@schapter` 不设 running mark，`\rightmark` 残留前置章名。修法：`\chapter*{绪论}` 后加一行 `\markboth{绪论}{绪论}`（§9.13）。编号章 `\chapter{}` 经 ctex `\@chapter`→`\chaptermark` 自动设，无此问题。

**Q: 为什么页眉每页不一样（偶数页是论文题目、奇数页是章名）？是 bug 吗？**
A: 不是 bug，是 spec §2.5 明确规定：偶数页页眉为论文题目，奇数页页眉为一级标题，均五号宋体。这是双面打印装订的设计（奇偶页内容朝向不同），符合学位论文格式要求，无需修改。

**Q: 为什么我用了 `\footcite{key}` 但页下没有参考文献条目？**
A: case-2（页下注，§2.14 + §1.2.4）须用 `\footfullcite{key}`，不是 `\footcite{key}`。`\footcite` 在 numeric 样式下只渲染 `[N]` 上标编号，不产生页下完整 GB/T 7714 著录。把 `\footcite` 换成 `\footfullcite` 重编译即可（§9.3）。区别：`\cite`/`\supercite` = 正文/上标编号（读者按 N 查文末表）；`\footfullcite` = 页下完整著录（每页重置）。

**Q: 为什么同一句话对同一文献 `\footfullcite` 多次会产生多条相同完整脚注？这是 bug 吗？**
A: 不是 bug，是**写作规范**。case-2 每页重编号下，每个 `\footfullcite{key}` 是**独立引用事件**，重复调用即重复完整条目（参考论文 p16 亦如此）。模板**故意不去重**（B-2 自动去重方案经实测 6 风险全崩，含 biblatex 架构天花板 R5）。避免一句之内同源三连引；如需区分页码/论点，首引完整 + 后续手写「前注[N]，第M页」。详见 §9.14。
