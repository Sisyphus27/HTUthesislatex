...
fig_label_re = re.compile(r"图\s*([0-9一二三四五六七八九]+)[-‐-](\d+)")
tbl_label_re = re.compile(r"表\s*([0-9一二三四五六七八九]+)[-‐-](\d+)")
def median(xs):
    ...
    for i in range(doc.page_count):
        for b in doc[i].get_text("dict").get("blocks", []):
            ...
            for ln in b.get("lines", []):
                ...
                m = re.search(r"图\s*([0-9一二三四五六七八九]+)\s*[-‐-]\s*\d", line_text) or \
                    re.search(r"表\s*([0-9一二三四五六七八九]+)\s*[-‐-]\s*\d", line_text)
                if m:
