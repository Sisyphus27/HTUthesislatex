...
# ATDD-2.4-10: baselineskip still 20bp in .def (Story 2.5 scope, R-3)
test_baselineskip_unchanged() {
  [[ -f "htuthesis.def" ]] || return 1
  grep -q '\\def\\htu@body@baselineskip{20bp}' htuthesis.def 2>/dev/null
}
run_test "P0" "ATDD-2.4-10" "baselineskip still 20bp in .def (Story 2.5 scope, R-3)" test_baselineskip_unchanged
