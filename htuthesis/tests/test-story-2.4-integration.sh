...
# ATDD-2.4-24: total pages ~50 (+/- 3) — footer change must not alter page count (AC-9)
test_page_count() {
  if [[ ! -f "main.log" ]]; then
    return 1
  fi
...
run_test "P1" "ATDD-2.4-24" "total pages ~50 (+/- 3) (AC-9, no page-count regression)" test_page_count
